...
- Pin `textFormat: Text.PlainText` on chart panel Text items that render
  Yahoo- or backend-derived strings, preventing AutoText from interpreting
  remote HTML as rich text and loading remote images.
...
- Bar widget width stays stable while rotating: the slot is sized to the
  widest watchlist label so longer tickers (e.g. BTC-USD) no longer jump
