...
expected_file="$repo_root/omarchy/bin/omastonk-qs.sha256"
srcid_file="$repo_root/omarchy/bin/omastonk-qs.srcid"
...
and commit omarchy/bin/omastonk-qs, .sha256, and .srcid in the same
...
if ! nm "$bin" | grep 'omastonk_qs' >/dev/null; then
...
manifest_version="$(jq -r '.version // empty' "$repo_root/manifest.json")"
...
if [[ "${VERIFY_BUNDLE_SKIP_REBUILD:-}" == 1 ]]; then
  echo "verified: omarchy/bin/omastonk-qs is non-stripped, version $crate_version, hash $committed, source id $actual_srcid (rebuild skipped)"
  exit 0
fi
...
actual="$(sha256sum "$tmp/target/$target/release/omastonk-qs" | awk '{print $1}')"
