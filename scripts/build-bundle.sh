...
out="$repo_root/target/$target/release/omastonk-qs"
install -Dm755 "$out" "$repo_root/omarchy/bin/omastonk-qs"
(
  cd "$repo_root/omarchy/bin"
  sha256sum omastonk-qs > omastonk-qs.sha256
  srcid="$("$repo_root/scripts/bundle-source-id.sh")"
  printf '%s  src Cargo.toml Cargo.lock rust-toolchain.toml\n' "$srcid" > omastonk-qs.srcid
)

echo "bundled: omarchy/bin/omastonk-qs"
sha256sum "$repo_root/omarchy/bin/omastonk-qs"
echo "source-id: $(awk '{print $1}' "$repo_root/omarchy/bin/omastonk-qs.srcid")"
